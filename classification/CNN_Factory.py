    ...
    def get_class(self, use_attribution=False, att_max_value=0.8):
        class CNN_prior:
            ...
            def __init__(self, word_vector_size):
                ...
                if use_attribution:
                    misattribution_error += tf.reduce_sum(self.output * (pos_heads - 0.9) ** 2
                                                     + (1 - self.output) * (neg_heads - 0.9) ** 2)
                    att_reg = tf.reduce_sum(self.output * tf.nn.relu(self.pos_attribution - att_max_value)
                                            + (1 - self.output) * tf.nn.relu(self.neg_attribution - att_max_value))
