...
class CNN_Factory:
    def get_class(self, use_attribution=False, num_classes=2,att_max_value=0.8):
        class CNN_prior:
            def get_attribution(self):
                self.attributions = []
                for class_ in range(num_classes):
                    att = tf.reduce_sum(tf.gradients(self.pre_max_sum[:, class_], self.vectors)[0] * self.vectors, axis=2)
                    att = softmax_padding(att, self.padding, axis=1)
            ...
            def __init__(self, word_vector_size):
                ...
                with tf.name_scope("relevance"):
                    hidden = 150
                    self.relevance_weight = tf.Variable(0.01 * xavier((hidden, num_classes)))
                    self.relevance_bias = tf.Variable(0.0 * xavier((1, num_classes)))
                ...
                prediction_error = - tf.reduce_sum(tf.one_hot(tf.cast(self.output,tf.int32),num_classes)*tf.log(rel + 10 ** -5, name="log2rel"))

                heads = []
                for att in self.attributions:
                    heads.append(tf.reduce_sum(tf.multiply(att, self.user_terms), axis=1))
                ...
                if use_attribution:
                    misattribution_error += (self.h[tf.cast(self.output[0][0],tf.int32)][0] - 0.9) ** 2
                    att_reg = 0
                    for att in self.attributions:
                        att_reg += tf.reduce_sum(tf.nn.relu(att - att_max_value))

                    corrective_error = - tf.reduce_sum(tf.one_hot(tf.cast(self.output,tf.int32),num_classes) * tf.log(rel_masked + 10 ** -5, name="log2rel"))
            ...
            def run(self, docs):
                ...
                classes_correct = {i:0 for i in range(num_classes)}
                true = []
                pred = []
                for doc_s in [docs[i:i + 1] for i in range(0, len(docs), 1)]:
                    fd = self.get_feed_dict_multiple(doc_s)
                    l1 = sess.run([self.relevance], feed_dict=fd)
                    for ind, doc in enumerate(doc_s):
                        d = {
                            "rel": np.argmax(l1[0][ind]),
                            # "pos_att": l1[1][ind],
                            # "neg_att": l1[2][ind]
                        }
                        doc.pred_class = d["rel"]
                        doc.parameters = d
                        num_correct += 1*(doc.pred_class==doc.class_)
                        num_seen += 1
                        classes_seen[doc.class_] += 1
                        classes_correct[doc.class_]+=1*(doc.pred_class==doc.class_)
                        true.append(doc.class_)
                        pred.append(doc.pred_class)
                    if num_seen%1000==0:
                        print(num_correct/num_seen*100)
                        # print({i:classes_correct[i]/float(classes_seen[i]) for i in classes_seen})
                        # print(confusion_matrix(y_pred=pred,y_true=true))

        return CNN_prior
if __name__=="__main__":
    CLF = CNN_Factory().get_class(use_attribution=True)
    CLF(word_vector_size=300)