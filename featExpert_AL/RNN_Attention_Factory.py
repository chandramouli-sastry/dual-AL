...
def getMisattributionError(attentions, user_terms, att_max, regularize=False):
    ...
    if regularize:
        return -share + (F.relu((attentions-att_max))).sum()
    else:
        return -share

class RNN_Attention_Factory:
    def get_class(self, use_attribution=False, att_max=0.8):
        class RNN_Prior:
            ...
            def __init__(self,word_vector_size):
                ...
                def get_out(inputs,padding,only_pre_max=False,for_output=False,output=None):
                    ...
                    if use_attribution and grad_input_proxy is not None:
                ...
                def get_loss(inputs, padding, user_terms, expected):
                    if user_terms.sum()>0 and use_attribution:
                        self.gru.derivative_needed = True
                    else:
                        self.gru.derivative_needed = False
                    ...
                    attention_error = 0
                    if use_attribution and user_terms.sum()>0:
            ...
            def train(self, docs):
                ...
                for epoch in range(epochs):
                    ...
                    last_2.pop(0)
                    last_2.append(total_error)
                    if abs(last_2[-1]-last_2[-2])<10**-5:
                        print("")
                        print("breaking")
                        break
                    print(".",end="")
            ...
            def run(self,docs):
                ...
                for doc_s in [docs[i:i + 10] for i in range(0, len(docs), 10)]:
                    ...
                    for ind,doc in enumerate(doc_s):
                        ...
                        doc.parameters = d
                        num_seen +=1
                        num_correct += (doc.pred_class==doc.class_)*1
                    if num_seen%100 == 0:
